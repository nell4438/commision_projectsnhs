...
class User {
	...
	function find_user($id="",$user_name=""){
		global $mydb;
		$mydb->setQuery("SELECT * FROM ".self::$tblname." 
			WHERE ACCOUNT_ID = {$id} OR user_email = '{$user_name}'");
		$cur = $mydb->executeQuery();
		$row_count = $mydb->num_rows($cur);
		return $row_count;
	}
	static function userAuthentication($user_email,$h_pass){
		global $mydb;
		$mydb->setQuery("SELECT * FROM `useraccounts` WHERE `user_email` = '". $user_email ."' and `user_pass` = '". $h_pass ."'");
		$cur = $mydb->executeQuery();
		if($cur==false){
			die(mysql_error());
		}
		$row_count = $mydb->num_rows($cur);//get the number of count
		 ...
		 $user_found = $mydb->loadSingleResult();
		 	...
		 	$_SESSION['user_pass'] 		= $user_found->user_pass;
