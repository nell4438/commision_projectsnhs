...
function doConfirm(PHPMailer $mail)
{
	global $mydb;
	$sem = new Semester();
	...
	$sql = "SELECT `tblstudent`.*,tblsection.section_name FROM `tblstudent` LEFT JOIN tblsection on tblsection.section_id = tblstudent.section_id WHERE IDNO =" . $_GET['IDNO'];
	...
	// 
	$emailReceiver = $studcourse['email'];
	$studName= $studcourse['FNAME'];
	$studYearLevel= $studcourse['YEARLEVEL'];
	$studSection= $studcourse['section_name'];
	$subject = "SNHS Online Enrollment Website - Confirmation";
	$message = "";
	ob_start();
...
function doEdit()
{
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_GET['action']) && $_GET['action'] == 'doEdit' && isset($_GET['IDNO'])) {
		$studentId = $_GET['IDNO'];
		...
		$updateSql = "UPDATE tblstudent SET STRAND = '" . mysqli_real_escape_string($mydb->conn, $strand) . "' WHERE IDNO = '" . mysqli_real_escape_string($mydb->conn, $studentId) . "'";
		$res = mysqli_query($mydb->conn, $updateSql) or die(mysqli_error($mydb->conn));

		message("Section has been set!", "success");
		redirect("index.php?view=success&IDNO=" . $studentId);
		exit;
	}
}
