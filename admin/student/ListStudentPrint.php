...
// print_r($_POST);
$semesterHoldData = $_POST['semester'];
?>
<!DOCTYPE html>
<html>
  ...
  <div class="wrapper">
    ...
    <section class="invoice">
      ...
      <div class="row">
        ...
        <h5 align="center">
          <?php echo isset($_POST['year_level']) ? "Course/Year :" . $_POST['year_level'] . " || SY :" . $semesterHoldData : ''; ?>
        </h5>
      </div>
      <!-- info row -->
      <div class="row">
          ...
          <form action="controller.php?action=delete" Method="POST">
            <div class="table-responsive">
              <table id="tablet" class="table table-striped table-bordered table-hover table-responsive"
                ...
                <tbody>
                  <?php
                  $sql = "SELECT * FROM `tblstudent` s WHERE NewEnrollees=0 AND  s.SEMESTER  = '$semesterHoldData'";
                  if ($_POST['year_level']) {
                    $sql .= " AND s.YEARLEVEL = '" . $_POST['year_level'] . "'";
                  }
                  if ($_POST['section'] != 37) {
                    $sql .= " AND s.SECTION = '" . $_POST['section'] . "'";
                  }
                  $sql .= " ORDER BY s.YEARLEVEL, s.LNAME, s.FNAME";
                  // echo $sql;
                  $mydb->setQuery($sql);
