...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    if($new_password !== $confirm_password){
        ...
        $uid = $_GET['uid'] ?? "";
        $stmt = $conn->prepare("SELECT * FROM `useraccounts` where md5(`ACCOUNT_ID`) = ?");
        $stmt->bind_param('s', $uid);
        $stmt->execute();
        $result = $stmt->get_result();
        if($result->num_rows > 0){
            $data = $result->fetch_assoc();
            $user_pass = sha1($new_password);
            $update = $conn->query("UPDATE `useraccounts` set `user_pass` = '{$user_pass}'");
            if($update){
