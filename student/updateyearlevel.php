...
// print_r($res);
$mydb->setQuery("SELECT * FROM  `tblsemester` WHERE SETSEM = 1");
$cur = $mydb->loadResultList();
$latestSemester = $cur[0]->SEMESTER;
// print_r($latestSemester);
$studdetails = new StudentDetails();
$details = $studdetails->single_StudentDetails($_SESSION['IDNO']);
?>

<form action="student/controller.php?action=editNewEnrollees" class="form-horizontal " method="post">
	<div class="table-responsive">
		<div class="col-md-8">
			<h2>Update Accounts</h2>
		</div>
		<div class="col-md-4"><label>Academic Year - <?= $latestSemester ?></label></div>
		<table class="table">
			<tr>
				<td><label>Id</label></td>
				<td>
					<input class="form-control input-md" readonly id="IDNO" name="IDNO" placeholder="Student Id"
						type="text" value="<?php echo isset($_SESSION['IDNO']) ? $_SESSION['IDNO'] : '' ?>">
				</td>
				<td colspan="4"></td>
			...
			<tr>
				...
				<td>
					<input required="true" class="form-control input-md" id="FNAME" name="FNAME"
						placeholder="First Name" type="text" value="<?php echo $res->FNAME; ?>">
				</td>
			...
			<tr>
				...
				<td>
					<input name="oldYearLevel" type="hidden" value="<?php echo $res->YEARLEVEL ?>">
					<input name="oldsemester" type="hidden" value="<?php echo $res->SEMESTERL ?>">
					<input name="newsemester" type="hidden" value="<?= $latestSemester ?>">
					<select class="form-control input-sm" id="YEARLEVEL" name="YEARLEVEL">
						<option value="37">Select</option>
						<?php
						for ($i = 7; $i <= 12; $i++) {
							$isSelected = ($i == $res->YEARLEVEL) ? 'selected' : '';
			...
			<tr>
				...
				<td colspan="6">
					<input required="true" class="form-control input-md" id="USER_NAME" name="USER_NAME"
						placeholder="Email Address" type="text" value="<?php echo $res->email; ?>">
				</td>
			...
			<tr>
				...
				<td colspan="2">
					<input required="true" class="form-control input-md" id="GUARDIAN" name="GUARDIAN"
						placeholder="Parents/Guardian Name" type="text" value="<?php echo $details->GUARDIAN; ?>">
				</td>
				<td><label>Contact No.</label></td>
				<td colspan="2"><input required="true" class="form-control input-md" id="GCONTACT" name="GCONTACT"
						placeholder="Contact Number" type="text" value="<?php echo $details->GCONTACT; ?>"></td>
			</tr>
